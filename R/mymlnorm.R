...
#' @title My Maximum Likelyhood Normal
#'
#' @param x is the sample vector
#' @param mu  mean of the of the pop
#' @param sig signma of the pop
#' @param ... extra stuff for the graph, used in contour
#'
#' @returns list of data, and a plot
#' @export
#'
#' @examples
#' mymlnormFunc(x = c(10, 12, 13, 15, 12, 11, 10), mu = seq(10, 15, length = 1000), sig = seq(0.1, 4, length = 1000), lwd = 2, labcex = 1)
mymlnormFunc = function(x, mu, sig,...){  #x sample vector
  ...
  lfun = function(x, m, p){
    log(dnorm(x, mean = m, sd = p)) # log lik for normal
  }
